#!/bin/bash
# Isolate AOE 5 flow scope: mint N world-state (PASS 00 + template 01),
# taxi flow -- next rc=1 on dirty 01, occupant writes PASS 01,
# taxi rc=0, unlock flow/02-scope.md.
# Real FLOW_BIN=flow.sh. FLOW_HARNESS_DISABLE=1. No /bin/true. No set -x.
# Do not source or exec hop, 1910, 0043, 0227, 0242, judge, or next scripts.
...
JOURNAL_COPY_TMP=/tmp/dory-aoe5-flow-scope-journal.jsonl
JOURNAL_COPY_RECEIPT="$REPO_ROOT/plans/reports/260828-ensure-aoe5-flow-scope-journal.jsonl"
SCOPE_SHA_RECEIPT="$REPO_ROOT/plans/reports/260828-ensure-aoe5-flow-scope-02.sha256"
...
case "$0" in
  *dory-flock-hop.sh|*dory-isolate-flow-sit.sh|*dory-isolate-flock-roster.sh|*dory-isolate-flock-report.sh|*dory-isolate-flock-prompt.sh|*dory-isolate-aoe5-flow-judge.sh|*dory-isolate-aoe5-flow-next.sh)
    echo "refuse: this script is dory-isolate-aoe5-flow-scope.sh" >&2
    exit 1
    ;;
esac
...
pat = re.compile(
    ...
    r"dory-isolate-aoe5-flow-judge|dory-isolate-aoe5-flow-next)",
    re.M,
)
if pat.search(text):
...
copy_scope_sha() {
  local f="$ISO_REAL/flow/02-scope.md"
  ...
  {
    sha256sum "$f"
    sha256sum "$SCOPE_TEMPLATE"
    stat -c 'path=%n ino=%i size=%s mtime=%Y' "$f"
  } > "$SCOPE_SHA_RECEIPT" || return 1
}
...
research_still_fail() {
  python3 - "$ISO_REAL/flow/01-research.md" <<'PY'
from pathlib import Path
import sys
text = Path(sys.argv[1]).read_text(encoding="utf-8")
if "- [ ]" in text or "[FILL" in text:
    raise SystemExit(0)
raise SystemExit("01-research.md is no longer FAIL")
PY
}
...
if not isinstance(stdout, str):
    stdout = str(stdout)
if "FAIL: gate for stage 01-research is not clean" not in stdout:
    raise SystemExit("taxi1 stdout missing FAIL gate 01-research not clean")
...
if not isinstance(stdout, str):
    stdout = str(stdout)
needle = "unlocked stage 2 (flow/02-scope.md)"
if needle not in stdout:
    raise SystemExit("taxi2 stdout missing unlocked stage 2")
...
scope_sha_ok() {
  python3 - "$ISO_REAL/flow/02-scope.md" "$SCOPE_TEMPLATE" <<'PY'
import hashlib, pathlib, sys
def sha(p):
    return hashlib.sha256(pathlib.Path(p).read_bytes()).hexdigest()
got, want = sha(sys.argv[1]), sha(sys.argv[2])
if got != want:
    raise SystemExit(f"02 sha {got} != template {want}")
text = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8")
if "[FILL" not in text:
    raise SystemExit("02-scope.md was filled")
PY
}
...
Operators sitting Dory cannot prove Flow unlocked scope after a real research gate.
N paid unlock of 01-research.md template; next must mint flow/02-scope.md.
This fixture keeps 00 clean so isolate can fail then unlock stage 2.
...
poll_pass_file() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    if [ -f "$ISO_REAL/flow/01-research.md" ] && cmp -s "$ISO_REAL/flow/01-research.md" "$PASS_WANT"; then
      return 0
    fi
    sleep 2
  done
...
ISO="$(mktemp -d "$CACHE/aoe5s.XXXXXX")" || fail "mktemp isolate"
...
PASS_00="$ISO_REAL/.aoe5s-pass-00"
PASS_WANT="$ISO_REAL/.aoe5s-pass-want"
write_pass_00
write_pass_want
...
cp -f "$RESEARCH_TEMPLATE" "$ISO_REAL/flow/01-research.md"
if [ -L "$ISO_REAL/flow/01-research.md" ]; then
  fail "research dest is symlink"
fi
research_still_fail || fail "minted 01 is not FAIL"
if cmp -s "$ISO_REAL/flow/01-research.md" "$PASS_WANT"; then
  fail "factory minted PASS 01"
fi
if [ -e "$ISO_REAL/home/.omp" ] || [ -e "$ISO_REAL/home/.agents" ]; then
  fail "isolate home grew an omp store"
fi
if [ -e "$ISO_REAL/flow/02-scope.md" ]; then
  fail "factory minted 02-scope.md"
fi
...
journal_taxi1 || fail "taxi 1 journal not fail-scope"
research_still_fail || fail "01 flipped PASS before occupant"
if [ -f "$ISO_REAL/flow/02-scope.md" ]; then
  fail "02-scope.md exists after taxi 1"
fi
if ! cmp -s "$ISO_REAL/flow/00-idea.md" "$PASS_00"; then
  fail "00-idea.md left PASS_00 after taxi 1"
fi
if cmp -s "$ISO_REAL/flow/01-research.md" "$PASS_WANT"; then
  fail "01 is PASS after taxi 1"
fi
...
if ! poll_pass_file; then
  ...
  if ! poll_pass_file; then
    echo "01-research.md after poll:" >&2
    cat "$ISO_REAL/flow/01-research.md" >&2 || true
...
journal_taxi2 || fail "taxi 2 journal not fail-then-scope"
if [ ! -f "$ISO_REAL/flow/02-scope.md" ]; then
  fail "02-scope.md missing after taxi 2"
fi
scope_sha_ok || fail "02 sha != template"
if ! cmp -s "$ISO_REAL/flow/00-idea.md" "$PASS_00"; then
  fail "00-idea.md left PASS_00 after taxi 2"
fi
if ! cmp -s "$ISO_REAL/flow/01-research.md" "$PASS_WANT"; then
  fail "01-research.md left PASS after taxi 2"
fi
if [ "$(repo_dory_stat)" != "$SNAP_DORY" ]; then
...
SCOPE_SHA="$(sha256sum "$ISO_REAL/flow/02-scope.md" | awk '{print $1}')"
TEMPLATE_SHA="$(sha256sum "$SCOPE_TEMPLATE" | awk '{print $1}')"

copy_journal || fail "copy journal before wipe"
copy_scope_sha || fail "copy 02 sha before wipe"
teardown
TORN=1
if sock_connectable "$FACTORY_XDG/dory/default/dory.sock"; then
...
[ -f "$SCOPE_SHA_RECEIPT" ] || fail "02 sha receipt copy missing"
...
echo "SCOPE_SHA=$SCOPE_SHA"
echo "TEMPLATE_SHA=$TEMPLATE_SHA"
echo "SCOPE_SHA_COPY=$SCOPE_SHA_RECEIPT"
echo "VISIBLE_MATCH=1"
echo "aoe5-scope=PASS"
exit 0
