#!/bin/bash
# Isolate AOE 5 flow prd: mint O world-state (PASS 00 + PASS 01 + template 02),
# taxi flow -- next rc=1 on dirty 02, occupant writes PASS 02,
# taxi rc=0, unlock flow/03-prd.md.
# Real FLOW_BIN=flow.sh. FLOW_HARNESS_DISABLE=1. No /bin/true. No set -x.
# Do not source or exec hop, 1910, 0043, 0227, 0242, judge, next, or scope scripts.
...
JOURNAL_COPY_TMP=/tmp/dory-aoe5-flow-prd-journal.jsonl
JOURNAL_COPY_RECEIPT="$REPO_ROOT/plans/reports/260829-ensure-aoe5-flow-prd-journal.jsonl"
PRD_SHA_RECEIPT="$REPO_ROOT/plans/reports/260829-ensure-aoe5-flow-prd-03.sha256"
SCOPE_TEMPLATE=/home/manhquy/.claude/skills/flow/_templates/02-scope.md
PRD_TEMPLATE=/home/manhquy/.claude/skills/flow/_templates/03-prd.md
...
case "$0" in
  *dory-flock-hop.sh|*dory-isolate-flow-sit.sh|*dory-isolate-flock-roster.sh|*dory-isolate-flock-report.sh|*dory-isolate-flock-prompt.sh|*dory-isolate-aoe5-flow-judge.sh|*dory-isolate-aoe5-flow-next.sh|*dory-isolate-aoe5-flow-scope.sh)
    echo "refuse: this script is dory-isolate-aoe5-flow-prd.sh" >&2
    exit 1
    ;;
esac
...
pat = re.compile(
    ...
    r"dory-isolate-aoe5-flow-judge|dory-isolate-aoe5-flow-next|dory-isolate-aoe5-flow-scope)",
    re.M,
)
if pat.search(text):
...
if "FLOW_HARNESS_DISABLE=1" not in text:
    raise SystemExit("self missing FLOW_HARNESS_DISABLE pin")
PY
}
...
copy_prd_sha() {
  local f="$ISO_REAL/flow/03-prd.md"
  ...
  {
    sha256sum "$f"
    sha256sum "$PRD_TEMPLATE"
    stat -c 'path=%n ino=%i size=%s mtime=%Y' "$f"
  } > "$PRD_SHA_RECEIPT" || return 1
}
...
scope_still_fail() {
  python3 - "$ISO_REAL/flow/02-scope.md" <<'PY'
from pathlib import Path
import sys
text = Path(sys.argv[1]).read_text(encoding="utf-8")
if "- [ ]" in text or "[FILL" in text:
    raise SystemExit(0)
raise SystemExit("02-scope.md is no longer FAIL")
PY
}
...
if not isinstance(stdout, str):
    stdout = str(stdout)
if "FAIL: gate for stage 02-scope is not clean" not in stdout:
    raise SystemExit("taxi1 stdout missing FAIL gate 02-scope not clean")
...
if not isinstance(stdout, str):
    stdout = str(stdout)
needle = "unlocked stage 3 (flow/03-prd.md)"
if needle not in stdout:
    raise SystemExit("taxi2 stdout missing unlocked stage 3")
...
if "unlocked stage 1 " in stdout:
    raise SystemExit("taxi2 stdout has unlocked stage 1")
if "unlocked stage 2 " in stdout:
    raise SystemExit("taxi2 stdout has unlocked stage 2")
PY
}

prd_sha_ok() {
  python3 - "$ISO_REAL/flow/03-prd.md" "$PRD_TEMPLATE" <<'PY'
import hashlib, pathlib, sys
def sha(p):
    return hashlib.sha256(pathlib.Path(p).read_bytes()).hexdigest()
got, want = sha(sys.argv[1]), sha(sys.argv[2])
if got != want:
    raise SystemExit(f"03 sha {got} != template {want}")
text = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8")
if "[FILL" not in text:
    raise SystemExit("03-prd.md was filled")
PY
}
...
Operators sitting Dory cannot prove Flow unlocked PRD after a real scope gate.
O paid unlock of 02-scope.md template; next must mint flow/03-prd.md.
This fixture keeps 00 clean so isolate can fail then unlock stage 3.

## One real person/group with this pain

Founder sitting the Dory desk after isolate N next unlock.
EOF
}
...
Mechanical isolate fixture. Not semantic gate-rules.md.
...
1. Dory isolate N — unlocks 01 template only
2. flow.sh scan_gate — mechanical boxes and FILL
3. CHARTER taxi — stranger flow.sh, no Dory next button

## What users say (web: 3 real complaints quoted+linked · non-web: real first-party friction)

1. > Founder: empty default sock is not sau
2. > Factory: leftover 5 stay mint
3. > Eval 1743: N paid; next world-state is 02
...
- isolate script time vs leftover fold vs wait lock

### The first-10-users channel (web) · who-benefits (non-web/internal)

isolate factory chair after N

### Why switch (vs the status quo)

mechanical unlock-2 after N. Not company Phase 5.

## Technically free vs hard

- Free (solved by libraries/platforms): next copies template
- Hard (custom work, real risk): semantic gate-rules and default sit
EOF
}
...
Scope = features chosen by IMPACT x COST. Mechanical isolate fixture.
...
- isolate unlock-3 — impact H (project advance after O) — grade A

## Suggested features (impact-first — proposed, not decided)

- leftover fold — impact L — grade A — OUT: doors mint

## Cut list (NOT in v1 — deferred, not deleted)

- company Phase 5 / card / semantic / default sit

## Decision

GO — mechanical unlock-3 after O. Not company Phase 5.
EOF
}
...
poll_pass_file() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    if [ -f "$ISO_REAL/flow/02-scope.md" ] && cmp -s "$ISO_REAL/flow/02-scope.md" "$PASS_WANT"; then
      return 0
    fi
    sleep 2
  done
...
ISO="$(mktemp -d "$CACHE/aoe5p.XXXXXX")" || fail "mktemp isolate"
...
PASS_00="$ISO_REAL/.aoe5p-pass-00"
PASS_01="$ISO_REAL/.aoe5p-pass-01"
PASS_WANT="$ISO_REAL/.aoe5p-pass-want"
write_pass_00
write_pass_01
write_pass_want
cp -f "$PASS_00" "$ISO_REAL/flow/00-idea.md"
if [ -L "$ISO_REAL/flow/00-idea.md" ]; then
  fail "idea dest is symlink"
fi
if ! cmp -s "$ISO_REAL/flow/00-idea.md" "$PASS_00"; then
  fail "factory did not mint PASS 00"
fi
cp -f "$PASS_01" "$ISO_REAL/flow/01-research.md"
if [ -L "$ISO_REAL/flow/01-research.md" ]; then
  fail "research dest is symlink"
fi
if ! cmp -s "$ISO_REAL/flow/01-research.md" "$PASS_01"; then
  fail "factory did not mint PASS 01"
fi
cp -f "$SCOPE_TEMPLATE" "$ISO_REAL/flow/02-scope.md"
if [ -L "$ISO_REAL/flow/02-scope.md" ]; then
  fail "scope dest is symlink"
fi
scope_still_fail || fail "minted 02 is not FAIL"
if cmp -s "$ISO_REAL/flow/02-scope.md" "$PASS_WANT"; then
  fail "factory minted PASS 02"
fi
if [ -e "$ISO_REAL/home/.omp" ] || [ -e "$ISO_REAL/home/.agents" ]; then
  fail "isolate home grew an omp store"
fi
if [ -e "$ISO_REAL/flow/03-prd.md" ]; then
  fail "factory minted 03-prd.md"
fi
...
journal_taxi1 || fail "taxi 1 journal not fail-prd"
scope_still_fail || fail "02 flipped PASS before occupant"
if [ -f "$ISO_REAL/flow/03-prd.md" ]; then
  fail "03-prd.md exists after taxi 1"
fi
if ! cmp -s "$ISO_REAL/flow/00-idea.md" "$PASS_00"; then
  fail "00-idea.md left PASS_00 after taxi 1"
fi
if ! cmp -s "$ISO_REAL/flow/01-research.md" "$PASS_01"; then
  fail "01-research.md left PASS_01 after taxi 1"
fi
if cmp -s "$ISO_REAL/flow/02-scope.md" "$PASS_WANT"; then
  fail "02 is PASS after taxi 1"
fi
...
if ! poll_pass_file; then
  ...
  if ! poll_pass_file; then
    echo "02-scope.md after poll:" >&2
    cat "$ISO_REAL/flow/02-scope.md" >&2 || true
...
journal_taxi2 || fail "taxi 2 journal not fail-then-prd"
if [ ! -f "$ISO_REAL/flow/03-prd.md" ]; then
  fail "03-prd.md missing after taxi 2"
fi
prd_sha_ok || fail "03 sha != template"
if ! cmp -s "$ISO_REAL/flow/00-idea.md" "$PASS_00"; then
  fail "00-idea.md left PASS_00 after taxi 2"
fi
if ! cmp -s "$ISO_REAL/flow/01-research.md" "$PASS_01"; then
  fail "01-research.md left PASS_01 after taxi 2"
fi
if ! cmp -s "$ISO_REAL/flow/02-scope.md" "$PASS_WANT"; then
  fail "02-scope.md left PASS after taxi 2"
fi
if [ "$(repo_dory_stat)" != "$SNAP_DORY" ]; then
...
PRD_SHA="$(sha256sum "$ISO_REAL/flow/03-prd.md" | awk '{print $1}')"
TEMPLATE_SHA="$(sha256sum "$PRD_TEMPLATE" | awk '{print $1}')"

copy_journal || fail "copy journal before wipe"
copy_prd_sha || fail "copy 03 sha before wipe"
teardown
TORN=1
if sock_connectable "$FACTORY_XDG/dory/default/dory.sock"; then
...
[ -f "$PRD_SHA_RECEIPT" ] || fail "03 sha receipt copy missing"
...
echo "PRD_SHA=$PRD_SHA"
echo "TEMPLATE_SHA=$TEMPLATE_SHA"
echo "PRD_SHA_COPY=$PRD_SHA_RECEIPT"
