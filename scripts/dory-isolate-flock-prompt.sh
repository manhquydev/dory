#!/bin/bash
# Isolate flock prompt after report: 0227 words + 0043 sibling.
# Exit 0 iff visible done words AND sibling workspace list AND factory
# sock not connectable. agent_prompt must not stall after report.
# No set -x. Do not source or exec hop, 1910, 0043, or 0227 scripts.
...
case "$0" in
  *dory-flock-hop.sh|*dory-isolate-flow-sit.sh|*dory-isolate-flock-roster.sh|*dory-isolate-flock-report.sh)
    echo "refuse: this script is dory-isolate-flock-prompt.sh" >&2
    exit 1
    ;;
esac
...
ISO="$(mktemp -d "$CACHE/prompt.XXXXXX")" || fail "mktemp isolate"
...
PROMPT_OUT="$(iso_mut agent prompt coord --timeout 180000 -- "$(cat <<'EOF'
...
EOF
  ...
  echo "$PROMPT_OUT" >&2
  case "$PROMPT_OUT" in
    *agent_prompt_stalled*) fail "sibling agent_prompt_stalled" ;;
  esac
  fail "sibling prompt cli failed"
}
case "$PROMPT_OUT" in
  *agent_prompt_stalled*) fail "sibling agent_prompt_stalled in body" ;;
esac
