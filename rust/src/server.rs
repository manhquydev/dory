...
fn tick_pane_wait(
    ...
    if hit {
        ...
        Some(envelope::success(&format!(
            "{{\"pane\":{{\"id\":{},\"pane_id\":{}}},\"matched\":true,\"text\":{}{}}}",
        ...
        Some(ws) => {
            ...
            for tab in &ws.tabs {
                for pane in &tab.panes {
                    ...
                    let cwd = proc_cwd(pid, &world.cwd);
                    out.push_str(&format!(
                        "{{\"id\":\"{}\",\"pid\":{},\"cwd\":{},\"occupant\":{},\"focused\":{},\"tab_id\":{},\"workspace_id\":{},\"pane_id\":{}}}",
                        pane.id,
                        pid,
                        envelope::json_string(&cwd.to_string_lossy()),
...
fn split_pane(
    ...
    match ratio {
        Some(r) => envelope::success(&format!(
            "{{\"pane\":{{\"id\":\"{new_id}\",\"pane_id\":\"{new_id}\"}},\"direction\":\"{dir}\",\"occupant\":null,\"ratio\":{}}}",
            crate::layout::clamp_ratio(r)
        )),
        None => envelope::success(&format!(
            "{{\"pane\":{{\"id\":\"{new_id}\",\"pane_id\":\"{new_id}\"}},\"direction\":\"{dir}\",\"occupant\":null}}"
        )),
    }
}
...
fn write_pane(world: &mut World, pane_id: &str, text: &str, raw: bool) -> String {
    ...
    match pane.held.write_all(&bytes) {
        Ok(()) => envelope::success(&format!("{{\"pane\":{{\"id\":\"{pane_id}\",\"pane_id\":\"{pane_id}\"}}}}")),
        Err(err) => envelope::runtime_error(&err.to_string()),
    }
}
    ...
    for ws in &world.workspaces {
        ...
        for tab in &ws.tabs {
            ...
            for pane in &tab.panes {
                ...
                items.push(',');
                items.push_str(&format!(
                    "{{\"k\":\"p\",\"id\":{},\"occ\":{},\"st\":{},\"cwd\":{},\"focused\":{},\"workspace_id\":{},\"tab_id\":{},\"pane_id\":{}}}",
                    envelope::json_string(&pane.id),
...
fn desk_snapshot(world: &World) -> String {
    ...
    let occupant = locate_pane(world, &focused)
        ...
        .unwrap_or_else(|| "null".to_string());
    let pid = locate_pane(world, &focused)
        .map(|loc| {
            world.workspaces[loc.wi].tabs[loc.ti].panes[loc.pi]
                .held
                .child_pid()
    ...
    let workspace_id = locate_pane(world, &focused)
        .map(|loc| world.workspaces[loc.wi].id.clone())
        .unwrap_or_else(|| String::new());
    let mut inner = format!(
        "{{\"focused\":{},\"focused_pane_id\":{},\"pane_id\":{},\"tab_id\":{},\"workspace_id\":{},\"cwd\":{},\"occupant\":{occupant},\"pid\":{pid},\"text\":{},",
...
fn desk_layout(
    ...
    for (i, cell) in cells.iter().enumerate() {
        ...
        json.push_str(&format!(
            "{{\"id\":{},\"x\":{},\"y\":{},\"w\":{},\"h\":{},\"occ\":{},\"st\":{},\"pane_id\":{},\"focused\":{}}}",
...
mod tests {
    ...
    fn tab_pane_list_get_are_inspect() {
        ...
        assert!(after_body.contains("\"pane_id\":"), "{after_body}");
        assert!(
            after_body.contains(&format!("\"pane_id\":\"{root}\"")),
            "{after_body}"
        );
        assert!(
            after_body.contains(&format!("\"pane_id\":\"{new_pane}\"")),
            "{after_body}"
        );
    ...
    fn desk_tree_pane_includes_cwd() {
        ...
        assert!(
            pane_obj[..pane_end].contains("\"pane_id\":"),
            "{tree}"
        );
    ...
    #[test]
    fn desk_layout_includes_tab_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let lay = rpc(&sock, r#"{"op":"desk.layout","cols":80,"rows":22}"#);
        assert!(lay.contains("\"ok\":true"), "{lay}");
        assert!(lay.contains("\"tab_id\":"), "{lay}");
    ...
    #[test]
    fn desk_layout_cell_includes_focused() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let lay = rpc(&sock, r#"{"op":"desk.layout","cols":80,"rows":22}"#);
        ...
        let focused = json_field(&lay, "focused");
        if id == focused {
            assert!(cell.contains("\"focused\":true"), "{lay}");
        } else {
            assert!(cell.contains("\"focused\":false"), "{lay}");
        }
        let _ = stop_server(&xdg);
        let _ = server.wait();
        let _ = fs::remove_dir_all(&xdg);
    ...
    #[test]
    fn desk_neighbor_includes_pane_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let snap = rpc_op(&sock, "snapshot");
        ...
        assert!(got.contains("\"ok\":true"), "{got}");
        assert!(got.contains("\"pane_id\":"), "{got}");
        let id = json_field(&got, "id");
        assert!(
            got.contains(&format!("\"pane_id\":\"{id}\"")),
            "{got}"
        );
        let _ = stop_server(&xdg);
        let _ = server.wait();
    ...
    #[test]
    fn split_pane_includes_pane_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let caller = json_field(&rpc_op(&sock, "snapshot"), "pane").to_string();
        ...
        let pane_end = pane_obj.find('}').expect("pane close");
        let nested = &pane_obj[..pane_end];
        assert!(nested.contains("\"id\":"), "{split}");
        assert!(nested.contains("\"pane_id\":"), "{split}");
        let id = json_field(nested, "id");
        assert!(
            nested.contains(&format!("\"pane_id\":\"{id}\"")),
            "{split}"
    ...
    #[test]
    fn write_pane_includes_pane_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let caller = json_field(&rpc_op(&sock, "snapshot"), "pane").to_string();
        let written = rpc(
            &sock,
            &format!(r#"{{"op":"pane.write","pane":"{caller}","text":"true"}}"#),
        );
        assert!(written.contains("\"ok\":true"), "{written}");
        let pane_start = written.find("\"pane\":{").expect("pane obj");
        let pane_obj = &written[pane_start + 8..];
        let pane_end = pane_obj.find('}').expect("pane close");
        let nested = &pane_obj[..pane_end];
        assert!(nested.contains("\"id\":"), "{written}");
        assert!(nested.contains("\"pane_id\":"), "{written}");
        let id = json_field(nested, "id");
        assert!(
            nested.contains(&format!("\"pane_id\":\"{id}\"")),
            "{written}"
    ...
    fn wait_pane_includes_pane_id() {
        ...
        let written = rpc(
            &sock,
            &format!(
                r#"{{"op":"pane.write","pane":"{caller}","text":"echo WAITPANEID"}}"#
            ),
        );
        assert!(written.contains("\"ok\":true"), "{written}");
        let waited = rpc(
            &sock,
            &format!(
                r#"{{"op":"pane.wait","pane":"{caller}","match":"WAITPANEID","timeout":5000}}"#
            ),
        );
        assert!(waited.contains("\"ok\":true"), "{waited}");
        assert!(waited.contains("\"matched\":true"), "{waited}");
        let pane_start = waited.find("\"pane\":{").expect("pane obj");
        let pane_obj = &waited[pane_start + 8..];
        let pane_end = pane_obj.find('}').expect("pane close");
        let nested = &pane_obj[..pane_end];
        assert!(nested.contains("\"id\":"), "{waited}");
        assert!(nested.contains("\"pane_id\":"), "{waited}");
        let id = json_field(nested, "id");
        assert!(
            nested.contains(&format!("\"pane_id\":\"{id}\"")),
            "{waited}"
    ...
    #[test]
    fn snapshot_includes_pane_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let snap = rpc_op(&sock, "snapshot");
        assert!(snap.contains("\"live\":true"), "{snap}");
        assert!(snap.contains("\"pane\":"), "{snap}");
        assert!(snap.contains("\"pane_id\":"), "{snap}");
        let pane = json_field(&snap, "pane");
        assert!(
            snap.contains(&format!("\"pane_id\":\"{pane}\"")),
            "{snap}"
        );
        let _ = stop_server(&xdg);
        let _ = server.wait();
    ...
    fn snapshot_includes_tab_id() {
        ...
        let tab = json_field(&snap, "tab");
        assert!(
            snap.contains(&format!("\"tab_id\":\"{tab}\"")),
            "{snap}"
        );
        let _ = stop_server(&xdg);
        let _ = server.wait();
    ...
    #[test]
    fn snapshot_includes_workspace_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let snap = rpc_op(&sock, "snapshot");
        assert!(snap.contains("\"live\":true"), "{snap}");
        assert!(snap.contains("\"workspace_id\":"), "{snap}");
    ...
    #[test]
    fn desk_snapshot_includes_focused_pane_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
    ...
    #[test]
    fn desk_snapshot_includes_pid() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let snap = rpc_op(&sock, "desk.snapshot");
        assert!(snap.contains("\"ok\":true"), "{snap}");
        assert!(snap.contains("\"pid\":"), "{snap}");
        let focused = json_field(&snap, "focused").to_string();
        let got = rpc(
            &sock,
            &format!(r#"{{"op":"pane.get","pane":"{focused}"}}"#),
        ...
        assert!(got.contains("\"ok\":true"), "{got}");
        let rest = got.split_once("\"pid\":").unwrap().1;
        let pid = rest.split(|c: char| !c.is_ascii_digit()).next().unwrap();
        assert!(
            snap.contains(&format!("\"pid\":{pid}")),
            "{snap} vs {got}"
        );
        let _ = stop_server(&xdg);
        let _ = server.wait();
    ...
    #[test]
    fn desk_snapshot_includes_workspace_id() {
        let xdg = temp_xdg();
        let mut server = start_server(&xdg);
        let sock = session_sock(&xdg);
        let snap = rpc_op(&sock, "desk.snapshot");
        ...
        assert!(snap.contains("\"workspace_id\":"), "{snap}");
        let live = rpc_op(&sock, "snapshot");
        let workspace_id = json_field(&snap, "workspace_id");
        let live_workspace_id = json_field(&live, "workspace_id");
        assert_eq!(workspace_id, live_workspace_id, "{snap} vs {live}");
        assert!(!workspace_id.is_empty(), "{snap}");
        let _ = stop_server(&xdg);
        let _ = server.wait();
        let _ = fs::remove_dir_all(&xdg);
    }
