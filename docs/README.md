...
## Paid occupant desk.snapshot cwd

Cite land with `git show HEAD:rust/src/server.rs`. Working-tree leftover `server.rs` is not the owner. Land wrap stays `desk.snapshot`. Occupant parse `.result.cwd`. Land `cwd` is `proc_cwd` on the focused pane child (`/proc/{pid}/cwd` with `world.cwd` fallback). Keep `.result.focused` / `.result.focused_pane_id` / `.result.text` / workspace list. There is no new RPC. There is no `dory snapshot` CLI. There is no `tab.get`. JSON stays `{"op":"desk.snapshot"}`. Do not recook live `snapshot` cwd. Do not recook desk.snapshot focused_pane_id. Do not recook pane.get nested pane_id. Do not change `dead_snapshot`. Do not claim `foreground_cwd`. Do not claim `pane.zoom`. Do not claim Darwin occupant `done`/`idle`.
...
## Paid occupant desk.snapshot pid

Cite land with `git show HEAD:rust/src/server.rs`. Working-tree leftover `server.rs` is not the owner. Land wrap stays `desk.snapshot`. Occupant parse `.result.pid`. Land `pid` is `held.child_pid()` on the focused pane (0 if no locate). Keep `.result.focused` / `.result.focused_pane_id` / `.result.cwd` / `.result.occupant` / `.result.text` / workspace list. There is no new RPC. There is no `dory snapshot` CLI. There is no `tab.get`. JSON stays `{"op":"desk.snapshot"}`. Do not recook live `snapshot` pid. Do not recook desk.snapshot occupant/cwd/focused_pane_id. Do not recook pane.get nested pane_id. Do not change `dead_snapshot`. Do not claim `foreground_cwd`. Do not claim `pane.zoom`. Do not claim Darwin occupant `done`/`idle`.

| Landing | Owner |
|---|---|
| Occupant desk.snapshot pid | `git show HEAD:rust/src/server.rs` |
| Skill desk.snapshot pid | `git show HEAD:skills/dory/SKILL.md` |


## Paid occupant desk.snapshot pane_id

Cite land with `git show HEAD:rust/src/server.rs`. Working-tree leftover `server.rs` is not the owner. Land wrap stays `desk.snapshot`. Occupant parse `.result.pane_id`. Land `pane_id` is `world.focused` (same as `.result.focused` / `.result.focused_pane_id`). Keep `.result.focused` / `.result.focused_pane_id` / `.result.cwd` / `.result.occupant` / `.result.pid` / `.result.text` / workspace list. There is no new RPC. There is no `dory snapshot` CLI. There is no `tab.get`. JSON stays `{"op":"desk.snapshot"}`. Do not recook live `snapshot` pane_id. Do not recook desk.snapshot pid/occupant/cwd/focused_pane_id. Do not recook pane.get nested pane_id. Do not wrap `pane` as an object. Do not change `dead_snapshot`. Do not claim `foreground_cwd`. Do not claim `pane.zoom`. Do not claim Darwin occupant `done`/`idle`.

| Landing | Owner |
|---|---|
| Occupant desk.snapshot pane_id | `git show HEAD:rust/src/server.rs` |
| Skill desk.snapshot pane_id | `git show HEAD:skills/dory/SKILL.md` |

## Paid occupant desk.snapshot tab_id

Cite land with `git show HEAD:rust/src/server.rs`. Working-tree leftover `server.rs` is not the owner. Land wrap stays `desk.snapshot`. Occupant parse `.result.tab_id`. Land `tab_id` is `tab.id` of the tab that contains the focused pane (empty string if no locate). Keep `.result.focused` / `.result.focused_pane_id` / `.result.pane_id` / `.result.cwd` / `.result.occupant` / `.result.pid` / `.result.text` / workspace list. There is no new RPC. There is no `dory snapshot` CLI. There is no `tab.get`. JSON stays `{"op":"desk.snapshot"}`. Do not recook live `snapshot` tab_id. Do not recook desk.snapshot pane_id/pid/occupant/cwd/focused_pane_id. Do not recook pane.get nested pane_id. Do not add `tab_id` on `pane.get`. Do not wrap `pane` as an object. Do not change `dead_snapshot`. Do not claim `foreground_cwd`. Do not claim `pane.zoom`. Do not claim Darwin occupant `done`/`idle`.

| Landing | Owner |
|---|---|
| Occupant desk.snapshot tab_id | `git show HEAD:rust/src/server.rs` |
| Skill desk.snapshot tab_id | `git show HEAD:skills/dory/SKILL.md` |

## Paid occupant desk.snapshot workspace_id

Cite land with `git show HEAD:rust/src/server.rs`. Working-tree leftover `server.rs` is not the owner. Land wrap stays `desk.snapshot`. Occupant parse `.result.workspace_id`. Land `workspace_id` is `ws.id` of the workspace that contains the focused pane (empty string if no locate). Keep `.result.focused` / `.result.focused_pane_id` / `.result.pane_id` / `.result.tab_id` / `.result.cwd` / `.result.occupant` / `.result.pid` / `.result.text` / workspace list. There is no new RPC. There is no `dory snapshot` CLI. There is no `tab.get`. JSON stays `{"op":"desk.snapshot"}`. Do not recook live `snapshot` workspace_id. Do not recook desk.snapshot tab_id/pane_id/pid/occupant/cwd/focused_pane_id. Do not recook pane.get nested pane_id. Do not add `workspace_id` on `pane.get`. Do not wrap `pane` as an object. Do not change `dead_snapshot`. Do not claim `foreground_cwd`. Do not claim `pane.zoom`. Do not claim Darwin occupant `done`/`idle`.
